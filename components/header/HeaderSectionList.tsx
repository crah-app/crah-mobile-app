...
interface HeaderSectionListProps<T> {
...
function HeaderSectionList<T>({
	...
	dataLoaded,
	errWhileLoading,
}: HeaderSectionListProps<T>) {
	const scrollOffset = useRef(0);
	const headerTranslateY = useRef(new Animated.Value(0)).current;
	const [isHeaderVisible, setIsHeaderVisible] = useState(true);
	const sectionListRef = useRef<SectionList<T>>(null);
		...
		<ThemedView theme={theme} flex={1} style={{ top }}>
			...
			{dataLoaded ? (
				errWhileLoading ? (
					...
					// @ts-ignore
					<SectionList
						ref={sectionListRef} // Referenz auf die FlatList
						data={data}
